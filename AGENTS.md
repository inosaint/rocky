...
A Codex personality plugin that makes Codex talk and think like Rocky, the Eridian engineer from *Project Hail Mary*. Two modes: **Talk** (communication style only) and **Full** (talk + engineering mind together).
...
- **SessionStart hook** (`hooks-handlers/session-start.sh`) — reads state from `~/.Codex/rocky-state.json` and extracts rules from the skill files (via `<!-- RULES:START/END -->` delimiters) to inject as `additionalContext`. This is the persistence layer.
...
State is stored globally at `~/.Codex/rocky-state.json`:

```json
{ "talk": false, "mind": false, "buddy": false }
```

Defaults to both `false` if the file is missing. Parsed using `python3` in the hook (Codex will prompt to allow this on first run — approve it).

## Testing Changes

Skills (`SKILL.md`) take effect immediately on next invocation. Hook changes (`session-start.sh`) require a **new Codex session** to take effect.
